...
vtkStandardNewMacro(vtkPolygon_81);

//----------------------------------------------------------------------------
// Instantiate polygon.
vtkPolygon_81::vtkPolygon_81()
{
...
//----------------------------------------------------------------------------
double vtkPolygon_81::ComputeArea()
{
  double normal[3]; //not used, but required for the
                    //following ComputeArea call
  return vtkPolygon_81::ComputeArea(this->GetPoints(),
...
//----------------------------------------------------------------------------
bool vtkPolygon_81::IsConvex()
{
  return vtkPolygon_81::IsConvex(this->GetPoints(),
                              this->GetNumberOfPoints(),
                              this->GetPointIds()->GetPointer(0));
}
...
// non-convex polygons.
void vtkPolygon_81::ComputeNormal(vtkPoints *p, int numPts, vtkIdType *pts,
                               double *n)
{
  int i;
...
// polygons.
void vtkPolygon_81::ComputeNormal(vtkIdTypeArray *ids, vtkPoints *p, double n[3])
{
  return vtkPolygon_81::ComputeNormal(p,ids->GetNumberOfTuples(),
                                   ids->GetPointer(0),n);
}
...
void vtkPolygon_81::ComputeNormal(vtkPoints *p, double *n)
{
  return vtkPolygon_81::ComputeNormal(p,p->GetNumberOfPoints(),nullptr,n);
}
...
void vtkPolygon_81::ComputeNormal (int numPts, double *pts, double n[3])
{
  int i;
  double *v1, *v2, *v3;
  double length;
...
bool vtkPolygon_81::IsConvex(vtkPoints *p, int numPts, vtkIdType *pts)
{
  int i;
...
//----------------------------------------------------------------------------
bool vtkPolygon_81::IsConvex(vtkIdTypeArray *ids, vtkPoints *p)
{
  return vtkPolygon_81::IsConvex(p, ids->GetNumberOfTuples(), ids->GetPointer(0));
}

//----------------------------------------------------------------------------
bool vtkPolygon_81::IsConvex(vtkPoints *p)
{
  return vtkPolygon_81::IsConvex(p, p->GetNumberOfPoints(), nullptr);
}

//----------------------------------------------------------------------------
int vtkPolygon_81::EvaluatePosition(double x[3], double* closestPoint,
...
//----------------------------------------------------------------------------
void vtkPolygon_81::EvaluateLocation(int& vtkNotUsed(subId), double pcoords[3],
...
// coordinate.
void vtkPolygon_81::InterpolateFunctions(double x[3], double *weights)
{
...
//----------------------------------------------------------------------------
// Compute interpolation weights using mean value coordinate.
void vtkPolygon_81::InterpolateFunctionsUsingMVC(double x[3], double *weights)
{
  int numPts=this->Points->GetNumberOfPoints();
...
int vtkPolygon_81::ParameterizePolygon(double *p0, double *p10, double& l10,
...
int vtkPolygon_81::PointInPolygon (double x[3], int numPts, double *pts,
...
// Triangulate polygon.
//
int vtkPolygon_81::Triangulate(vtkIdList *outTris)
{
...
//
int vtkPolygon_81::NonDegenerateTriangulate(vtkIdList *outTris)
{
  ...
  while(numPtsRemoved < numPts)
  {
    ...
    vtkPolygon_81 *polygon = vtkPolygon_81::New();
...
int vtkPolygon_81::BoundedTriangulate(vtkIdList *outTris, double tolerance)
{
...
int vtkPolygon_81::EarCutTriangulation ()
{
...
int vtkPolygon_81::UnbiasedEarCutTriangulation(int seed)
{
  vtkPolyVertexList poly(this->PointIds, this->Points,
                         this->Tolerance*this->Tolerance);
...
//----------------------------------------------------------------------------
int vtkPolygon_81::CellBoundary(int vtkNotUsed(subId), double pcoords[3],
                             vtkIdList *pts)
{
...
//----------------------------------------------------------------------------
void vtkPolygon_81::Contour(double value, vtkDataArray *cellScalars,
...
//----------------------------------------------------------------------------
vtkCell *vtkPolygon_81::GetEdge(int edgeId)
{
  int numPts=this->Points->GetNumberOfPoints();
...
//
int vtkPolygon_81::IntersectWithLine(double p1[3], double p2[3], double tol,double& t,
...
//----------------------------------------------------------------------------
int vtkPolygon_81::Triangulate(int vtkNotUsed(index), vtkIdList *ptIds,
                            vtkPoints *pts)
{
  int i, success;
  double *bounds, d;
...
void vtkPolygon_81::Derivatives(int vtkNotUsed(subId), double pcoords[3],
...
//----------------------------------------------------------------------------
void vtkPolygon_81::Clip(double value, vtkDataArray *cellScalars,
...
// is an intersection.
int vtkPolygon_81::IntersectPolygonWithPolygon(int npts, double *pts,double bounds[6],
                                            ...
                                            double x[3])
{
  ...
  //
  vtkPolygon_81::ComputeNormal(npts2, pts2, n);
  ...
  for (i=0; i<npts; i++)
  {
    ...
    if ( (vtkPlane::IntersectWithLine(p1,p2,n,pts2,t,x)) == 1 )
    {
      if ( (npts2==3
           ...
           || (npts2>3
               && vtkPolygon_81::PointInPolygon(x,npts2,pts2,bounds2,n)
               ==VTK_POLYGON_INSIDE))
      {
        return 1;
      }
  ...
  //
  vtkPolygon_81::ComputeNormal(npts, pts, n);
  ...
  for (i=0; i<npts2; i++)
  {
    ...
    if ( (vtkPlane::IntersectWithLine(p1,p2,n,pts,t,x)) == 1 )
    {
      if ( (npts==3 && vtkTriangle::PointInTriangle(x,pts,pts+3,pts+6,tol2))
           || (npts>3 && vtkPolygon_81::PointInPolygon(x,npts,pts,bounds,n)
               ==VTK_POLYGON_INSIDE))
      {
        return 1;
      }
...
// 3D space.
double vtkPolygon_81::ComputeArea(vtkPoints *p, vtkIdType numPts, vtkIdType *pts,
                               double n[3])
{
  if ( numPts < 3 )
  {
  ...
  else
  {
    ...
    vtkPolygon_81::ComputeNormal(p,numPts, pts, n);
...
//----------------------------------------------------------------------------
void vtkPolygon_81::PrintSelf(ostream& os, vtkIndent indent)
{
...
bool vtkPolygon_81::ComputeCentroid(vtkPoints *p,
                                 int numPts,
                                 vtkIdType *ids,
                                 double c[3])
{
...
bool vtkPolygon_81::ComputeCentroid(vtkIdTypeArray *ids,
                                 vtkPoints *p,
                                 double c[3])
{
  return vtkPolygon_81::ComputeCentroid(p,ids->GetNumberOfTuples(),
                                     ids->GetPointer(0),c);
}

//----------------------------------------------------------------------------
double vtkPolygon_81::DistanceToPolygon(double x[3], int numPts, double *pts,
                                     double bounds[6], double closest[3])
{
  ...
  {
    double n[3];
    vtkPolygon_81::ComputeNormal(numPts, pts, n);
    if ( vtkPolygon_81::PointInPolygon(x,numPts,pts,bounds,n) )
    {
      closest[0] = x[0];
      closest[1] = x[1];
      closest[2] = x[2];
...
//----------------------------------------------------------------------------
int vtkPolygon_81::IntersectConvex2DCells(vtkCell *cell1, vtkCell *cell2,
